export const PUJA_LIST = [
  {
    id: "satyanarayan-puja",
    name: "Satyanarayan Katha & Puja",
    hindiName: "श्री सत्यनारायण व्रत कथा एवं महाहवन",
    badge: "सर्वाधिक लोकप्रिय (Most Popular)",
    category: "Auspicious",
    image: "https://images.unsplash.com/photo-1609357605129-26f69add5d6e?auto=format&fit=crop&w=800&q=80",
    description: "Lord Satyanarayan is the embodiment of supreme truth and prosperity. Our 20+ years experienced North Indian Pandit Ji performs Gauri-Ganesh Pujan, Navagraha Sthapana, 5-chapter Katha path, and sacred Hawan.",
    price: 2499,
    priceWithSamagri: 3999,
    duration: "2 - 2.5 Hours",
    pandits: "1 Senior North Indian Vedic Pandit",
    samagriIncluded: [
      "Panchamrit (Milk, Curd, Ghee, Honey, Sugar)",
      "Gangajal, Roli, Akshat, Chandan, Haldi, Moli",
      "Paan leaves, Supari, Clove (Lavang), Cardamom (Elaichi)",
      "Navagraha Dhoop, Pure Kapoor, Janeu, Diya Batti",
      "Pure Desi Ghee & Hawan Samidha (Mango wood)",
      "Prasad Panjiri & Panchmeva preparation guidance",
      "Aarti Sangrah & Katha Pustak"
    ],
    vidhiHighlights: [
      "Swasti Vachan & Sankalpa in your name & gotra",
      "Gauri-Ganesh Sthapana & Shodashopachara Pujan",
      "Navagraha, Varuna (Kalash) & Sarvatobhadra Mandal invocation",
      "Recitation of Sri Satyanarayan Katha (5 Chapters) in pure Hindi",
      "Yajna Hawan with Gayatri & Mahamrityunjaya ahutis",
      "Karpura Aarti, Pushpanjali, and Charan-amrit distribution"
    ],
    benefits: [
      "Brings peace, unity, and abundance to family",
      "Removes ongoing obstacles and negative energies",
      "Fulfills cherished wishes (Manokamna Siddhi)",
      "Bestows auspicious beginnings for new ventures"
    ]
  },
  {
    id: "grihapravesh-puja",
    name: "Grihapravesh & Vastu Shanti",
    hindiName: "शुभ गृह प्रवेश, वास्तु शांति एवं हवन",
    badge: "ट्रेंडिंग (Trending)",
    category: "Housewarming",
    image: "https://images.unsplash.com/photo-1545232979-fbf68fe9b1af?auto=format&fit=crop&w=800&q=80",
    description: "Vedic housewarming ceremony to consecrate your new home, invoke Vastu Purusha, remove construction doshas, and fill every corner with divine positive vibrations.",
    price: 4999,
    priceWithSamagri: 7499,
    duration: "3 - 4 Hours",
    pandits: "1 or 2 Experienced Hindi / Bihari Pandits",
    samagriIncluded: [
      "Vastu Kalash, Copper Urn, Coconut with red cloth",
      "Mango leaves (Amra Pallav), Fresh Toran for main gate",
      "Navadhanya (9 Sacred Grains), Vastu Yantra",
      "Pure Desi Cow Ghee (1kg), Guggal, Loban, Camphor",
      "Cast Iron Hawan Kund / Vedic Bricks arrangement",
      "Red, Yellow & White Cloths for Sthapana",
      "Shodash Matrika & Navagraha Samagri Kit"
    ],
    vidhiHighlights: [
      "Dwar Puja & Auspicious Threshold invocation",
      "Auspicious Milk Boiling (Doodh Ubalna) ritual",
      "Gauri-Ganesh & Kalash Sthapana",
      "Vastu Purusha Awahan and directional blessing",
      "Navagraha Shanti Hawan with Vedic Mantras",
      "Ghar Parikrama with Holy Kalash water & conch shell (Shankhnaad)"
    ],
    benefits: [
      "Neutralizes Vastu Doshas of the newly acquired home",
      "Shields family members from ill health and negativity",
      "Ensures prosperity, peace, and longevity in the new premises",
      "Creates a sacred sanctuary filled with positive prana"
    ]
  },
  {
    id: "rudrabhishek-puja",
    name: "Maha Rudrabhishek Puja",
    hindiName: "महा रुद्राभिषेक एवं शिव पूजन",
    badge: "विशेष फलदायी (Most Powerful)",
    category: "Dosha Nivaran",
    image: "https://images.unsplash.com/photo-1583089892943-e02e5b017b6a?auto=format&fit=crop&w=800&q=80",
    description: "Direct invocation of Lord Shiva through sacred bathing with 11 holy dravyas and Shukla Yajurveda Rudri Path chants for supreme health, peace, and overcoming doshas.",
    price: 3499,
    priceWithSamagri: 4999,
    duration: "2 - 3 Hours",
    pandits: "1 Senior Vedic Pandit (Rudri Specialist)",
    samagriIncluded: [
      "Fresh Bilva Patra (108 Belpatra), Dhatura, Bhasma",
      "Pure Cow Milk, Curd, Honey, Sugarcane juice, Gangajal",
      "Chandan, Attar, Akshat, White Madar / Lotus flowers",
      "Janeu, Rudraksha mala, Diya & pure ghee, Shivalinga stand"
    ],
    vidhiHighlights: [
      "Shiva Parvati Dhyana & Sankalpa",
      "Namakam & Chamakam recitation from Shukla Yajurveda",
      "11 Sacred Dravyas Abhishek on Shivalinga",
      "Mahamrityunjaya Mantra Japa & Bilvashtakam chanting",
      "Bhasma Arpan, Shringar & Maha Mangal Aarti"
    ],
    benefits: [
      "Destroys chronic illnesses and mental distress",
      "Dispels fear of premature death (Akal Mrityu)",
      "Protects against evil eye, malefic planets, and enemies",
      "Brings deep spiritual tranquility and inner bliss"
    ]
  },
  {
    id: "marriage-puja",
    name: "Vivah Sanskar / Marriage Ceremony",
    hindiName: "वैदिक विवाह संस्कार एवं सात फेरे",
    badge: "संपूर्ण वैदिक (Complete Vedic)",
    category: "Sanskar",
    image: "https://images.unsplash.com/photo-1583939003579-730e3918a45a?auto=format&fit=crop&w=800&q=80",
    description: "Solemnize the sacred union of bride and groom with authentic North Indian rituals: Jaimala, Kanyadaan, Panigrahana, Saptapadi (7 Pheras), Hawan, and Sindoor Daan.",
    price: 11999,
    priceWithSamagri: 16999,
    duration: "4 - 5 Hours",
    pandits: "2 or 3 Scholarly Vedic Acharyas",
    samagriIncluded: [
      "Vedic Mandap Pujan Kit, Sacred Wood, Camphor",
      "Kanyadaan Thali essentials, Silver/Brass coins",
      "Saptapadi Betel leaves, Supari, Yellow Rice",
      "Desi Ghee (2kg), Hawan Samagri, Dry fruits, Spices",
      "Gathbandhan Dupatta, Sindoor, Mangalsutra Pujan kit"
    ],
    vidhiHighlights: [
      "Var Aagman, Dwar Char & Madhuparka welcoming",
      "Gauri-Ganesh Pujan & Kanyadaan Sankalpa",
      "Laja Homam & Sacred Wedding Agni Hawan",
      "Saptapadi (Seven Vows explained in Hindi & Sanskrit)",
      "Sindoor Daan, Mangalsutra Pujan & Elders' Ashirwad"
    ],
    benefits: [
      "Binds the two souls in eternal harmony and mutual devotion",
      "Conducted strictly as per Sanatan Vedic scriptures",
      "Pandit Ji articulates meanings of all 7 sacred vows clearly",
      "A lifetime of divine blessings for a joyful marital life"
    ]
  },
  {
    id: "business-puja",
    name: "Office / Shop Opening Puja",
    hindiName: "दुकान / ऑफिस उद्घाटन एवं व्यापार वृद्धि",
    badge: "व्यापार वृद्धि (Prosperity)",
    category: "Auspicious",
    image: "https://images.unsplash.com/photo-1507679799987-c73779587ccf?auto=format&fit=crop&w=800&q=80",
    description: "Invoke Goddess Maha Lakshmi, Lord Ganesha, and Lord Kuber to bless your commercial venture, shop, startup, or corporate office with boundless growth and profit.",
    price: 3999,
    priceWithSamagri: 5999,
    duration: "2 - 2.5 Hours",
    pandits: "1 Experienced North Indian Pandit",
    samagriIncluded: [
      "Lakshmi-Ganesh Idol / Yantra, Kuber Yantra",
      "Bahi-Khata / Laptop & Ledger Pujan materials",
      "Kamalgatta, Yellow Cowries, Ashta Gandh, Coriander seeds",
      "Hawan Kund, Mango Wood, Pure Ghee, Sweets"
    ],
    vidhiHighlights: [
      "Vighnaharta Ganesh Pujan for smooth operations",
      "Maha Lakshmi Awahan & Sri Suktam chanting",
      "Kuber Pujan for continuous cash flow and profitability",
      "Vyapar Vriddhi Hawan with special samagri",
      "Locker, Desk, and Main Entrance blessing"
    ],
    benefits: [
      "Ensures business profitability and rapid customer growth",
      "Protects venture from commercial hazards and setbacks",
      "Fosters positive teamwork and productive work environment"
    ]
  },
  {
    id: "navagraha-puja",
    name: "Navagraha Shanti Puja & Hawan",
    hindiName: "नवग्रह शांति पूजन एवं महाहवन",
    badge: "ग्रह दोष निवारण (Planetary Peace)",
    category: "Dosha Nivaran",
    image: "https://images.unsplash.com/photo-1518709268805-4e9042af9f23?auto=format&fit=crop&w=800&q=80",
    description: "Align and propitiate the 9 planetary lords (Surya, Chandra, Mangal, Budh, Guru, Shukra, Shani, Rahu, Ketu) to alleviate planetary doshas, dasha hurdles, and career blockages.",
    price: 3499,
    priceWithSamagri: 5199,
    duration: "2.5 Hours",
    pandits: "1 Learned Vedic Astrologer & Pandit",
    samagriIncluded: [
      "Navadhanya (9 sacred grains), 9 Color cloths for grahas",
      "Nine specific samidha woods (Ark, Palash, Khadir, etc.)",
      "Sesame seeds (Til), Jau, Ghee, Guggal, Loban, Supari"
    ],
    vidhiHighlights: [
      "Navagraha Mandal creation with colored rice grains",
      "Individual Graha Beej Mantra chanting",
      "108 ahutis in sacred fire for afflicted planets",
      "Shanti Path and protective Graha Kavach"
    ],
    benefits: [
      "Mitigates Sade Sati, Manglik Dosha, and Rahu-Ketu transit effects",
      "Enhances career advancements, educational focus, and peace",
      "Harmonizes home relations"
    ]
  },
  {
    id: "mahamrityunjaya-jaap",
    name: "Maha Mrityunjaya Jaap & Hawan",
    hindiName: "महामृत्युंजय जप एवं आरोग्य रक्षा",
    badge: "आरोग्य व दीर्घायु (Health & Life)",
    category: "Dosha Nivaran",
    image: "https://images.unsplash.com/photo-1544717305-2782549b5136?auto=format&fit=crop&w=800&q=80",
    description: "An extraordinarily potent Vedic ritual for life rejuvenation, recovery from serious ailments, shielding against unexpected accidents, and mental vitality.",
    price: 4499,
    priceWithSamagri: 6999,
    duration: "3 - 3.5 Hours",
    pandits: "1 or 2 Vedic Acharyas",
    samagriIncluded: [
      "Giloy, Belpatra, Black sesame, Pure cow ghee, Guggal",
      "Ayurvedic medicinal herbs, Durva grass, Honey",
      "Maha Mrityunjaya Yantra, Sacred Raksha Sutra"
    ],
    vidhiHighlights: [
      "Devotee Sankalpa for health and recovery",
      "1,100 / 2,100 / 5,100 Japa of Maha Mrityunjaya Mantra",
      "Amrit Sanjeevani Hawan with Giloy stems",
      "Tying of energized Raksha Kavach on wrist"
    ],
    benefits: [
      "Speeds up medical recovery and strengthens immunity",
      "Overcomes chronic stress, insomnia, and fear of illness",
      "Bestows longevity and divine protection"
    ]
  },
  {
    id: "diwali-laxmi-puja",
    name: "Diwali & Maha Lakshmi Puja",
    hindiName: "दीपावली महालक्ष्मी एवं कुबेर पूजन",
    badge: "त्योहार विशेष (Festival Special)",
    category: "Auspicious",
    image: "https://images.unsplash.com/photo-1508963493744-76fce69379c0?auto=format&fit=crop&w=800&q=80",
    description: "Celebrate the festival of lights with full North Indian Vidhi: Sri Suktam path, Kanakadhara Stotram, Kuber Pujan, and lighting of 108 Deepaks.",
    price: 2999,
    priceWithSamagri: 4499,
    duration: "2 Hours",
    pandits: "1 Experienced North Indian Pandit",
    samagriIncluded: [
      "Kamal Gatta, Lotus flowers, Gold/Silver coin vidhi",
      "Laxmi-Ganesh Pratima, Kheel, Batashe, Panchmeva",
      "Diwali Deepam essentials, Pure Desi Ghee"
    ],
    vidhiHighlights: [
      "Deepak Jyoti Pujan & Kalash Sthapana",
      "Sri Suktam and Lakshmi Sahasranama chanting",
      "Kuber Khazana Pujan & Aabharan (Jewelry) Pujan",
      "Maha Aarti with 108 Deepaks & Shanti Path"
    ],
    benefits: [
      "Invites perennial abundance and wealth into household",
      "Removes debts and financial anxieties",
      "Creates an aura of joy and celebration"
    ]
  },
  {
    id: "sundarkand-path",
    name: "Akhand Sundarkand & Hanuman Chalisa",
    hindiName: "सुंदरकांड पाठ एवं हनुमान चौकी",
    badge: "संकट मोचन ( संकट निवारक)",
    category: "Auspicious",
    image: "https://images.unsplash.com/photo-1606293926075-69a00dbfde81?auto=format&fit=crop&w=800&q=80",
    description: "Melodious, devotional recitation of Sri Ramcharitmanas Sundarkand. Fills home with pure devotion, removes all distress, fear, and negativity.",
    price: 3499,
    priceWithSamagri: 4999,
    duration: "2.5 - 3 Hours",
    pandits: "1 or 2 Kirtan & Path Specialists",
    samagriIncluded: [
      "Sindoor, Chameli Oil, Hanuman Chola, Dhoop, Diya",
      "Besan Laddoos / Boondi prasad, Tulsi Dal, Bananas",
      "Ramayana Granth, Hawan essentials"
    ],
    vidhiHighlights: [
      "Ram Durbar Sthapana & Hanuman Awahan",
      "Musical Sundarkand recitation in pure Hindi",
      "Hanuman Chalisa & Sankat Mochan Path",
      "Hanuman Ji Chola Arpan & Hawan Aarti"
    ],
    benefits: [
      "Instills courage, self-confidence, and fearlessness",
      "Banishes black magic, evil forces, and nightmares",
      "Brings Lord Hanuman's direct grace upon the family"
    ]
  },
  {
    id: "namkaran-puja",
    name: "Namkaran Sanskar (Baby Naming)",
    hindiName: "नामकरण संस्कार एवं जातकर्म",
    badge: "संस्कार (Sacred Sanskar)",
    category: "Sanskar",
    image: "https://images.unsplash.com/photo-1519689680058-324335c77eba?auto=format&fit=crop&w=800&q=80",
    description: "Bestow an auspicious Vedic name to your newborn based on Janma Nakshatra and planetary degrees, accompanied by Ayushya Hawan for radiant health.",
    price: 2999,
    priceWithSamagri: 4499,
    duration: "2 Hours",
    pandits: "1 Jyotish & Vedic Pandit",
    samagriIncluded: [
      "Bronze plate (Kansa thali), Honey & Gold quill vidhi",
      "Navagraha samagri, New white & red cloth for baby",
      "Hawan samagri, Desi Ghee, Kalash items, Sweets"
    ],
    vidhiHighlights: [
      "Ayushya Hawan for child's health & longevity",
      "Nakshatra & Rashi calculation by Pandit Ji",
      "Whispering the sacred name in baby's right ear",
      "Family blessings and gift sanctification"
    ],
    benefits: [
      "Connects the child with cosmic positive vibrations",
      "Blesses child with intelligence, health, and bright future",
      "Memorable traditional family milestone"
    ]
  },
  {
    id: "bhoomi-puja",
    name: "Bhoomi Puja & Foundation Stone",
    hindiName: "भूमि पूजन एवं नींव शिलान्यास",
    badge: "निर्माण सुरक्षा (Construction)",
    category: "Housewarming",
    image: "https://images.unsplash.com/photo-1541888946425-d0fbb186c5f7?auto=format&fit=crop&w=800&q=80",
    description: "Appease Mother Earth (Bhumi Devi) and Vastu Purusha before laying the foundation stone of your new home, plot, or commercial building.",
    price: 4999,
    priceWithSamagri: 7499,
    duration: "2.5 - 3 Hours",
    pandits: "1 Vedic Shastra Pandit",
    samagriIncluded: [
      "Pancharatna, 5 Bricks, Navadhanya, Copper pot",
      "Naga-Nagin pair in silver/copper for earth blessing",
      "Hawan Samidha, Ghee, Coconuts, Flowers, Vermilion"
    ],
    vidhiHighlights: [
      "Digging first auspicious corner (Ishanya corner)",
      "Bhumi Devi & Shesha Naga Awahan",
      "Foundation Stone (Shila) consecration",
      "Bhoomi Hawan & Shanti Paath"
    ],
    benefits: [
      "Ensures smooth, accident-free construction work",
      "Neutralizes past blemishes or negative energy of land",
      "Invites long-term stability and fortune"
    ]
  },
  {
    id: "pitru-dosh-tarpan",
    name: "Pitra Dosh Nivaran & Tarpan",
    hindiName: "पितृ दोष शांति, श्राद्ध एवं तर्पण",
    badge: "पितृ तृप्ति (Ancestral Peace)",
    category: "Dosha Nivaran",
    image: "https://images.unsplash.com/photo-1509198397868-475647b2a1e5?auto=format&fit=crop&w=800&q=80",
    description: "Offer peace and liberation to ancestral spirits (Pitrus). Clears ancestral debts, resolves hurdles in childbirth, marriage, and family discord.",
    price: 3499,
    priceWithSamagri: 5299,
    duration: "2 - 2.5 Hours",
    pandits: "1 Senior Vedic Karmakandi Pandit",
    samagriIncluded: [
      "Kusha grass ring (Pavitri), Black sesame (Kala Til), Jau",
      "Gangajal, Cow Milk, Honey, White flowers, Janeu",
      "Pind Daan rice flour preparation & Hawan items"
    ],
    vidhiHighlights: [
      "Sankalp with Gotra and Father's ancestral lineage",
      "Tarpan vidhi with sacred water & black sesame",
      "Pind Daan & Pitra Gayatri Ahuti in Hawan",
      "Brahmin Bhojan sankalp and Gau grass offering"
    ],
    benefits: [
      "Grants supreme peace to departed ancestors",
      "Resolves unexplained family disputes and health worries",
      "Restores ancestral blessings for the lineage"
    ]
  }
];

export const TESTIMONIALS = [
  {
    id: 1,
    name: "Madhusmita Rout",
    location: "Whitefield, Bangalore",
    ceremony: "Satyanarayan Puja",
    rating: 5,
    text: "During the preparation for the puja, Pandit Ji demonstrated a deep understanding of the rituals and traditions. The katha explanation in Hindi was so soulful that my entire family was captivated. 100% recommended for authentic North Indian puja!",
    avatar: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?auto=format&fit=crop&w=150&q=80"
  },
  {
    id: 2,
    name: "Niharika Singh",
    location: "Bellandur, Bangalore",
    ceremony: "Grihapravesh Puja",
    rating: 5,
    text: "Highly recommend Sri North Hindi Pandit Ji for grihapravesh puja. Panditji brought every single item of puja samagri with him. The hawan and milk-boiling vidhi were performed with immense care and devotion. Truly exceptional experience!",
    avatar: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80"
  },
  {
    id: 3,
    name: "Amlan Ghosh",
    location: "Koramangala, Bangalore",
    ceremony: "Rudrabhishek & Hawan",
    rating: 5,
    text: "I would like to share my heartfelt feedback for the best North Indian Pandit in Bangalore. His knowledge of Vedic Sanskrit chants and precise recitation of Rudri Path created an unforgettable divine energy in our home.",
    avatar: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=150&q=80"
  },
  {
    id: 4,
    name: "Rajesh Sharma & Family",
    location: "HSR Layout, Bangalore",
    ceremony: "Vivah Sanskar (Wedding)",
    rating: 5,
    text: "Pandit Ji conducted our son's wedding pheras with utmost dignity. He explained the meaning of each of the seven vows in easy Hindi, which both families deeply appreciated. No hidden demands, completely transparent!",
    avatar: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=150&q=80"
  }
];

export const FAQS = [
  {
    question: "Can we book an authentic North Indian Pandit online?",
    hindiQuestion: "क्या हम ऑनलाइन नॉर्थ इंडियन पंडित जी बुक कर सकते हैं?",
    answer: "Yes, absolutely! Through our platform, you can select your desired puja, pick your preferred date and time, and book verified North Indian (Hindi, UP, Bihari) Pandits with 20+ years experience. We provide instant booking confirmation."
  },
  {
    question: "Do you provide complete Puja Samagri or do we have to buy it?",
    hindiQuestion: "क्या पूजा की सारी सामग्री पंडित जी लेकर आएंगे?",
    answer: "We offer both options! With our 'All-Inclusive Samagri' package, Pandit Ji brings 100% pure, unadulterated Vedic samagri (Desi Ghee, Hawan Kund, Gangajal, Navadhanya, Camphor, Dhoop, Prasad essentials). Alternatively, if you already have samagri, you can book Pandit-only services."
  },
  {
    question: "What languages do the Pandits speak?",
    hindiQuestion: "पंडित जी किस भाषा में बात और मंत्रोच्चार करेंगे?",
    answer: "Our Pandits are scholarly Brahmins from Varanasi, Mathura, Ayodhya, Prayagraj, and Bihar. They chant mantras in authentic Vedic Sanskrit and communicate fluently in Hindi, Bhojpuri, Maithili, and English so your family understands every ritual."
  },
  {
    question: "How is the Shubh Muhurat (auspicious time) decided?",
    hindiQuestion: "पूजा का शुभ मुहूर्त कैसे तय किया जाता है?",
    answer: "Once you submit your booking inquiry, our senior Acharya contacts you personally. They analyze your Janma Rashi, Gotra, and the Panchang to recommend the most auspicious date and Choghadiya/Muhurat for your ceremony without any extra astrological consultation charge."
  },
  {
    question: "Are there any hidden costs or extra dakshina demands?",
    hindiQuestion: "क्या कोई छुपा हुआ चार्ज या बाद में दक्षिणा की मांग होती है?",
    answer: "Never. We believe in complete transparency and sanctity. The quoted price includes complete Pandit dakshina, travel within city limits, and all agreed samagri. No surprise demands are ever made on the day of puja."
  },
  {
    question: "What if I need to reschedule or cancel my booking?",
    hindiQuestion: "अगर तारीख बदलनी हो तो क्या नियम हैं?",
    answer: "We understand that plans can change. You can reschedule your puja date anytime up to 24 hours prior to the ceremony at zero penalty. Just inform us via WhatsApp or phone call."
  }
];
